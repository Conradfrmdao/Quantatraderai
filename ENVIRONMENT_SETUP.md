# QuantatraderAI — Complete Environment Variables Setup Guide
...
you deploy the Next.js frontend (e.g. `https://quantatraderai.vercel.app`).
...
5. Replace `YOUR_CLERK_DOMAIN` with your Clerk domain (looks like `clerk.quantatraderai.com` or `grateful-fox-42.clerk.accounts.dev`)
...
2. Click **+ Create Project** → choose **Python** → name it "QuantatraderAI Backend"
...
**How to get it:** Your Vercel deployment URL (e.g. `https://quantatraderai.vercel.app`)
...
4. Name it `QuantatraderAI Production`
...
```
EMAIL_FROM=QuantatraderAI <noreply@yourdomain.com>
```

**How to get it:**
1. In Resend → **Domains** → **Add Domain**
2. Enter your domain (e.g. `quantatraderai.com`)
...
```
EMAIL_FROM=QuantatraderAI <onboarding@resend.dev>
```
...
2. Name it `QuantatraderAI Frontend`
3. Copy the DSN
...
*Generated by QuantatraderAI setup. Last updated: May 2026.*

---

## Part 3 — LLM Providers (the AI brain)

This is what makes the trading decisions. You have multiple options — free tiers exist
for all of them. Here is exactly how the system uses them.
...
Single provider mode (default):
  Every tick → one LLM call → buy/sell/hold decision

AI Council mode (PRO plan, ENABLE_COUNCIL=true):
  Every tick → 3 parallel LLM calls (Groq + Claude + Gemini)
  → majority vote → if deadlock, abstain → much higher accuracy
```

Your current setup: **Groq as primary + Claude as fallback.**
That is a good production setup. Groq is free and fast. Claude is the quality fallback.
...
```bash
LLM_PROVIDER=groq              # ✅ set — uses Groq as primary
LLM_MODEL=llama-3.3-70b-versatile  # ✅ set — best free Groq model
GROQ_API_KEY=gsk_...           # ✅ set
ANTHROPIC_API_KEY=sk-ant-...   # ✅ set — used as fallback + for SANITIZE_MODEL
```

**You are already operational.** The variables below are optional upgrades.

---

### Optional: Add Gemini (enables full AI Council for PRO users)

#### `GEMINI_API_KEY`
**What it does:** Adds Google Gemini as the 3rd council member.
Without it, the council runs with only 2 providers (Groq + Claude) — still works,
but 3 providers gives a cleaner majority vote.
...
**Which model it uses:** `gemini-2.0-flash` (fast, free, excellent quality)

---

### Optional: OpenRouter (access 100+ models with one key)

#### `OPENROUTER_API_KEY`
**What it does:** Unlocks access to every major LLM (GPT-4, DeepSeek, Mistral, Llama)
through a single API. Useful if you want to switch models without changing providers.
Has a generous free tier with no credit card required.
...
**Free models you can use** (set `LLM_MODEL` to one of these):
```
meta-llama/llama-3.3-70b-instruct:free   ← best quality free
deepseek/deepseek-r1:free                ← reasoning model, great for trading
google/gemma-2-9b-it:free               ← fast and cheap
mistralai/mistral-7b-instruct:free      ← lightweight
```

To switch to OpenRouter: set `LLM_PROVIDER=openrouter` in `.env`

---

### Optional: Ollama (run models locally — zero cost, full privacy)

#### `OLLAMA_BASE_URL`
**What it does:** Points the app at a local Ollama instance running on your machine
or a self-hosted server. Zero API costs, fully private, no rate limits.
...
**How to get it:**
1. Go to [ollama.com](https://ollama.com) → download and install
2. Run: `ollama pull llama3.3` (or any model)
3. Ollama starts automatically at `http://localhost:11434`
4. Set `LLM_PROVIDER=ollama` and `LLM_MODEL=llama3.3` in `.env`

**Best local models for trading:**
```
llama3.3       ← best quality, needs 32GB RAM
llama3.2       ← good quality, needs 16GB RAM  
qwen2.5:14b    ← excellent reasoning, 16GB RAM
```

Note: Local models are slower (2-5s per decision vs 0.3s Groq) and less accurate
than frontier models. Use for privacy or zero-cost self-hosted deployments.

---

### Optional: `SANITIZE_MODEL`

**What it does:** A second, cheaper model used to clean/validate the primary LLM's
JSON output before parsing it. Catches malformed responses.

```
SANITIZE_MODEL=claude-haiku-4-5-20251001
```

**How to get it:** You already have `ANTHROPIC_API_KEY` set so this just works.
Haiku is extremely cheap (~$0.0001 per sanitize call). Leave this as-is.

---

### Choosing your LLM strategy

| Setup | Cost | Speed | Quality | Best for |
|-------|------|-------|---------|----------|
| `groq` only (current) | **Free** | Very fast (0.3s) | Good | Testing, paper trading |
| `groq` + Claude fallback (current) | Near-free | Fast | Good+ | Early live trading |
| Council: Groq + Claude + Gemini | Near-free | 2-3s | **Best** | PRO plan users |
| `anthropic` only | ~$2-5/day | Medium | Excellent | High-stakes live |
| `openrouter` + free models | Free | Fast | Good | Budget production |
| `ollama` local | Free | Slow | Good | Privacy-first |

**Recommendation for launch:** Keep your current setup (`groq` primary + Anthropic fallback).
Add `GEMINI_API_KEY` to enable the full AI Council for PRO users — it's free.

---

### Full LLM section for `.env`

```bash
# ── LLM Primary provider ───────────────────────────────────────────────────────
LLM_PROVIDER=groq                        # groq | anthropic | gemini | openrouter | ollama
LLM_MODEL=llama-3.3-70b-versatile        # model name for chosen provider
...
# ADD THESE:
GEMINI_API_KEY=AIzaSy...                 # Free — enables full 3-provider AI Council
# OPENROUTER_API_KEY=sk-or-...           # Optional — uncomment if using OpenRouter
# OLLAMA_BASE_URL=http://localhost:11434  # Optional — uncomment if self-hosting

# ── AI Council (PRO plan feature) ─────────────────────────────────────────────
ENABLE_COUNCIL=false                     # Set to true to force-enable for all users
                                         # PRO plan users get it automatically

# ── Response sanitizer ────────────────────────────────────────────────────────
SANITIZE_MODEL=claude-haiku-4-5-20251001 # Already optimal — leave as-is

# ── Feature flags ─────────────────────────────────────────────────────────────
ENABLE_INTEL=false                       # MTF confluence + news sentiment (CPU heavy)
ENABLE_RAG=false                         # Trade memory (needs OPENAI_API_KEY for embeddings)
ENABLE_COPY_TRADING=false               # Mirror trades to followers
```

---

### One thing to know about costs

With your current setup (Groq as primary):

| Usage level | Estimated monthly LLM cost |
|-------------|---------------------------|
| 1 user, 1h ticks, paper | **$0** (Groq free tier) |
| 10 users, 1h ticks | **$0–2** (Groq free + Claude fallback rarely fires) |
| 100 users, 15m ticks | **~$15–30** (mix of Groq + Claude) |
| 100 users, AI Council ON | **~$50–80** (3× LLM calls per tick) |

Groq's free tier is 14,400 requests/day. At 1h ticks, 10 users = 240 requests/day.
You won't hit the free limit until ~60 users on 1h ticks. After that, Groq paid
tier is $0.59 per million tokens — still nearly free.
